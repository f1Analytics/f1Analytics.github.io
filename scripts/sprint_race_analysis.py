...
year, grand_prix, session = int(sys.argv[1]), sys.argv[2], 'Sprint'
...
# drivers_to_visualize = [i for i in sys.argv[3:]]
drivers_to_visualize = ['LEC', 'PER', 'VER', 'ALO', 'HAM']
...
for driver in drivers_to_visualize:
    ...
    poly = np.polyfit(driver_laps['LapNumber'], driver_laps['LapTimeSeconds'], 4)
    ...
    plt.savefig(f'{foldername}{year}_{circuit}_sprintracepace_comparison.png', dpi=300)
